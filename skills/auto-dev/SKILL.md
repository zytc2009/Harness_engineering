...
- `Acceptance Criteria`
- `Constraints`
- `workspace_dir` 或 `output_dir`（二选一，必须询问）
  - `workspace_dir`：有 git 管理需求时使用，pipeline 完成后自动 copy + commit
  - `output_dir`：只需要文件复制，无 git 需求时使用
...
- dependency_policy: ...
- forbidden_paths: ...
- workspace_dir: <git 仓库路径，相对项目根目录，如 ../my-project 或 output/my-feature>
# 或者，不需要 git 时：
- output_dir: <仅复制文件，如 output/my-feature>
